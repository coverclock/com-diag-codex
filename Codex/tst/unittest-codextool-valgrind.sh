...
FILES="${FILECLIENTSOURCE} ${FILECLIENTSINK} ${FILESERVERSOURCE} ${FILESERVERSINK}"
...
valgrind --show-error-list=yes --leak-check=full codextool -i -b ${BUFFERSIZE} -C ${CERT}/codextool-servercert.pem -K ${CERT}/codextool-serverkey.pem -P ${CERT} -n 0.0.0.0:${TUNNEL} < ${FILESERVERSOURCE} > ${FILESERVERSINK} &
PIDSERVER=$!
sleep 5

valgrind --show-error-list=yes --leak-check=full codextool -i -b ${BUFFERSIZE} -C ${CERT}/codextool-clientcert.pem -K ${CERT}/codextool-clientkey.pem -P ${CERT} -f localhost:${TUNNEL} < ${FILECLIENTSOURCE} > ${FILECLIENTSINK} &
PIDCLIENT=$!
sleep 5
