...
while [[ ${CLIENTS} -gt 0 ]]; do
    ...
    sleep 1
    dd if=/dev/urandom bs=${BLOCKSIZE} count=${BLOCKS} iflag=fullblock | functionaltest-handshake-client -C ${CRTPATH}/bogus.pem -K ${CRTPATH}/bogus.pem -f ${FAREND} -B ${BUFSIZE} -p ${PERIOD} > /dev/null &
    CLIENT="${CLIENT} $!"
    CLIENTS=$(( ${CLIENTS} - 1 ))
